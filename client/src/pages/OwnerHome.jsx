...
const OwnerHome = ({ user }) => {
  ...
  useEffect(() => {
    if (!user || !user.uid) {
      ...
      setError(null);      try {
        ...
        console.log("Attempting to fetch technician reservations with user ID:", user.uid, "and email:", user.email);
        
        // Create additional queries to get technician bookings specifically
        // Try both userId and uid fields since different forms might use different field names
        const technicianBookingsQuery = query(
          collection(db, "technicianreservations"),
          where("userId", "==", user.uid)
        );
        
        // Create a secondary query for technicianreservations with uid
        const technicianBookingsQueryAlt = query(
          collection(db, "technicianreservations"),
          where("uid", "==", user.uid)
        );
        
        // Also try to get technician bookings by user email
        const technicianEmailBookingsQuery = user.email ? query(
          collection(db, "technicianreservations"),
        ...
        const technicianSnapshot = await getDocs(technicianBookingsQuery);
        const technicianAltSnapshot = await getDocs(technicianBookingsQueryAlt);
          
        // If we have an email query, execute it too
        let technicianEmailSnapshot = null;
        if (technicianEmailBookingsQuery) {
          technicianEmailSnapshot = await getDocs(technicianEmailBookingsQuery);
        }
        
        console.log("Technician bookings found:", !technicianSnapshot.empty, technicianSnapshot.size);
        if (technicianEmailSnapshot) {
          console.log("Technician email bookings found:", !technicianEmailSnapshot.empty, technicianEmailSnapshot.size);
        }
            // Array to store all bookings
          const bookingsData = [];
          if (!querySnapshot.empty) {            querySnapshot.docs.forEach(doc => {
              const data = doc.data();
              ...
              if (data.serviceDate) {
                ...
                // Check if createdAt is a Timestamp or string
              ...
              // Normalize status field - make first letter uppercase
              ...
              const formattedBooking = {
                ...
                // Add service time if it doesn't exist
                serviceTime: data.serviceTime || '00:00'
              };

              // Push to bookings array
              bookingsData.push(formattedBooking);
            });          }          // Process technician bookings from primary query
          if (!technicianSnapshot.empty) {
            technicianSnapshot.docs.forEach(doc => {
              const data = doc.data();
              console.log('Raw technician booking from Firebase:', data);
              
              // Skip if this is a reference booking (has bookingId) and we already have the main booking
              if (data.bookingId && bookingsData.some(b => b.id === data.bookingId)) {
                console.log('Skipping duplicate technician booking with bookingId:', data.bookingId);
                return;
              }
              ...
              // Add formatted booking to bookingsData array
              const formattedBooking = {
                id: doc.id,
                ...data,
                date: formattedDate || '',
                ...
                type: data.serviceType || (data.message ? data.message.substring(0, 20) : 'General Service'),
                serviceType: data.serviceType || (data.message ? data.message.substring(0, 20) : 'General Service'),
                bookingType: 'technician', // Force booking type for technician bookings
                status: normalizedStatus,
                cost: typeof data.cost === 'number' ? data.cost : 0,
          ...
          // Process technician bookings from alt query 
          if (!technicianAltSnapshot.empty) {
            console.log('Processing technician alt bookings:', technicianAltSnapshot.size);
            technicianAltSnapshot.docs.forEach(doc => {
              const data = doc.data();
              console.log('Raw technician alt booking from Firebase:', data);
              
              // Skip if this is a reference booking (has bookingId) and we already have the main booking
              // or if it's a duplicate of a booking we already processed
              if ((data.bookingId && bookingsData.some(b => b.id === data.bookingId)) || 
                  bookingsData.some(b => b.id === doc.id)) {
                console.log('Skipping duplicate technician alt booking with bookingId:', data.bookingId);
                return;
              }
          ...
          // Process technician bookings from email query as well
          if (technicianEmailSnapshot && !technicianEmailSnapshot.empty) {
            console.log('Processing technician email bookings:', technicianEmailSnapshot.size);
            technicianEmailSnapshot.docs.forEach(doc => {
              const data = doc.data();
              console.log('Raw technician email booking:', data);
              
              // Skip if this is a reference booking (has bookingId) and we already have the main booking
              // or if it's a duplicate of a booking we already processed
              if ((data.bookingId && bookingsData.some(b => b.id === data.bookingId)) || 
                  bookingsData.some(b => b.id === doc.id)) {
                console.log('Skipping duplicate technician email booking:', doc.id);
                return;
              }
          ...
          console.log('All processed bookings data:', bookingsData);
          
          // Count by type for debugging
          const serviceCenterCount = bookingsData.filter(b => b.bookingType === 'service-center').length;
          const technicianCount = bookingsData.filter(b => b.bookingType === 'technician').length;
          console.log(`Booking counts - Service Centers: ${serviceCenterCount}, Technicians: ${technicianCount}`);
    ...
    fetchBookings();
  }, [user]);  // Filter bookings based on status and type
  const filteredBookings = bookings.filter((booking) => {
    ...
    // Make sure we correctly identify technician bookings (both 'technician' and any with technicianName)
    const isTechnicianBooking = booking.bookingType === "technician" || 
                               (booking.technicianName && booking.technicianName !== 'Unknown Technician');
    
    const isServiceCenterBooking = booking.bookingType === "service-center" || 
                                  (booking.serviceCenterName && booking.serviceCenterName !== 'Unknown Service Center');
                                  
    const matchesType = filterType === "All" || 
                       (filterType === "Service Center" && isServiceCenterBooking) ||
                       (filterType === "Technician" && isTechnicianBooking);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (typeof updatedService.id === 'string') {
        // Update the booking in main bookings collection
        const bookingRef = doc(db, "bookings", updatedService.id);
        ...
        console.log('Booking updated in main bookings collection');
        
        // Determine which reservations collection to update based on booking type
        const reservationsCollection = updatedService.bookingType === 'service-center' 
          ? 'servicereservations' 
        ...
        try {
          const reservationsQuery = query(
            collection(db, reservationsCollection),
          ...
          if (!reservationsSnapshot.empty) {
            ...
            await updateDoc(doc(db, reservationsCollection, reservationDoc.id), {
              serviceDate: updatedService.serviceDate,
              serviceTime: updatedService.serviceTime,
              serviceType: updatedService.serviceType,
              status: updatedService.status,
            ...
            console.log(`Booking also updated in ${reservationsCollection}`);
          }
        } catch (reservationError) {
          console.error(`Error updating reservation in ${reservationsCollection}:`, reservationError);
    ...
    if (confirm('Are you sure you want to delete this booking?')) {
      try {
        ...
        if (typeof id === 'string') {
          ...
          console.log('Booking deleted from main bookings collection');
          
          // Determine which reservations collection to update based on booking type
          const reservationsCollection = bookingType === 'service-center' 
            ? 'servicereservations' 
            : 'technicianreservations';
          
          // Try to find and delete the corresponding reservation
          try {
            const reservationsQuery = query(
              collection(db, reservationsCollection),
              where("bookingId", "==", id)
            ...
            if (!reservationsSnapshot.empty) {
              const reservationDoc = reservationsSnapshot.docs[0];
              await deleteDoc(doc(db, reservationsCollection, reservationDoc.id));
              console.log(`Booking also deleted from ${reservationsCollection}`);
            }
          } catch (reservationError) {
            console.error(`Error deleting reservation from ${reservationsCollection}:`, reservationError);
            // Continue execution even if this fails
          }
        }
  ...
  const technicianBookings = bookings.filter((b) => b.bookingType === "technician").length;    // Debug info - log booking types and statuses
  useEffect(() => {
      ...
      <div className="flex flex-1 relative z-10">
        ...
        <main className="flex-1 max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            </div>            <div className="p-6 rounded-lg shadow-lg text-center bg-white/10 backdrop-blur-md border border-gray-700/50 hover:border-red-500 transition-all duration-300">
                ...
                <tbody>                  {filteredBookings.length > 0 ? (
                    filteredBookings.map((booking) => (
                      <tr key={booking.id} className="hover:bg-gray-700/50">
                        ...
                        <td className="border border-gray-700/50 p-3 font-[Open Sans] text-gray-300">
                          {booking.serviceType}
                        </td>                        <td className="border border-gray-700/50 p-3 font-[Open Sans] text-gray-300">
          ...
          </section>          {/* Edit Service Modal */}
          {showEditModal && editService && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="p-6 rounded-lg shadow-xl w-full max-w-md bg-gray-800 text-white">                <h3 className="text-xl font-semibold mb-4 font-[Raleway] text-white">
                  Edit Booking
                </h3>
