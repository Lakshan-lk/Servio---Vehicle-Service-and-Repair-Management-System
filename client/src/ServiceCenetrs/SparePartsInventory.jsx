...
const SparePartsInventory = () => {
  ...
  const [editingId, setEditingId] = useState(null);  const [newPart, setNewPart] = useState({
    name: "",
    category: "",
    quantity: 0,
    price: 0,
  ...
  const handleInputChange = (e) => {
    ...
    try {
      if (isEditing) {
        ...
        const response = await sparePartsService.updateSparePart(editingId, newPart);
        
        if (response.success) {
          setParts(parts.map(part => 
            part.id === editingId ? { ...response.data } : part
          ));
          setIsEditing(false);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 relative z-10">
        <ServiceCenterSidebar activePath="/spare-parts-inventory" />        <main className="flex-1 max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          {/* Status Messages */}
          {successMessage && (
            <div className="bg-green-600/80 text-white p-4 rounded-lg mb-6 flex items-center justify-between shadow-lg backdrop-blur-sm">
          ...
          {error && (
            <div className="bg-red-600/80 text-white p-4 rounded-lg mb-6 flex items-center justify-between shadow-lg backdrop-blur-sm">
              <div className="flex items-center">
              ...
              <div className="p-6 rounded-lg shadow-xl w-full max-w-md bg-gray-800 text-white">
                ...
                <form onSubmit={handleAddOrUpdatePart} className="space-y-4 font-[Open Sans]">
                  ...
                  <div>
                    ...
                    <label className="block mb-1 text-gray-300">Part Number (optional)</label>
                    <input
                      type="text"
                      name="partNumber"
                  ...
                  <div>
                    <label className="block mb-1 text-gray-300">Manufacturer (optional)</label>
                    <input
                      type="text"
                      name="manufacturer"
                      ...
                      onChange={handleInputChange}
                      className="w-full p-2 border rounded-md bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-2 focus:ring-red-500"
                      placeholder="e.g., Toyota, Generic"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      className="w-full p-2 border rounded-md bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-2 focus:ring-red-500"
                      placeholder="Enter a description for this part"
                      rows="2"
                    />
                  </div>
